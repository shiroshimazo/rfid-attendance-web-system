...
async function installEnrollment() {
  await db.exec('reset role')
  for (const name of ['202609140002_teacher_confirmed_late.sql', '202609190001_subject_enrollment.sql']) {
    const sql = await readFile(new URL(`../supabase/migrations/${name}`, import.meta.url), 'utf8')
    await db.exec(sql.replace(/^begin;$/m, '').replace(/^commit;$/m, ''))
  }
}
const enroll = (id, students, version = 1) => rows('select public.save_subject_enrollment($1,$2::bigint[],$3)', [id, students, version])

test('enrollment bootstraps existing rosters once and preserves removals, history and RFID on reapply', async () => {
  const a = await schedule()
  await identity('teacher'); await confirm(a)
  await installEnrollment()
  const history = await rows('select * from public.subject_attendance')
  const checks = await rows(await readFile(new URL('../supabase/verify_subject_enrollment.sql', import.meta.url), 'utf8'))
  assert.equal(checks.length, 6)
  assert(checks.every(row => row.result === 'PASS'), JSON.stringify(checks))
  assert.deepEqual(await rows('select schedule_id,student_id,active from public.subject_enrollments'), [{ schedule_id:a, student_id:student, active:true }])
  await identity('admin'); await enroll(a, [])
  await installEnrollment()
  assert.equal((await rows('select active from public.subject_enrollments'))[0].active, false)
  assert.deepEqual(await rows('select * from public.subject_attendance'), history)
  assert.deepEqual(await dailyEvidence(), legacy)
  const newSchedule = await schedule(assignmentB, '09:00', '10:00')
  assert.deepEqual(await rows('select * from public.subject_enrollments where schedule_id=$1', [newSchedule]), [])
})

test('explicit enrollment supports cross-class students, rejects unassigned students and keeps tap evidence separate', async () => {
  const a = await schedule()
  await installEnrollment()
  await identity('admin'); await enroll(a, [otherStudent])
  await identity('teacher')
  assert.equal((await rows('select id from public.students where id=$1', [otherStudent])).length, 1)
  assert.equal((await rows('select student_id from public.attendance_records where student_id=$1', [otherStudent])).length, 1)
  await rejected(() => confirm(a, 'Present', student), /not enrolled/)
  await confirm(a, 'Absent', otherStudent)
  await identity('otherTeacher')
  assert.deepEqual(await rows('select * from public.subject_enrollments'), [])
  await rejected(() => confirm(a, 'Present', otherStudent))
  await identity('otherStudent')
  assert.equal((await rows('select id from public.subject_schedules where id=$1', [a])).length, 1)
  assert.equal((await rows('select * from public.subject_attendance'))[0].attendance_status, 'Absent')
  await identity('student')
  assert.equal((await rows('select id from public.subject_schedules where id=$1', [a])).length, 0)
  await identity('admin'); await enroll(a, [], 2)
  await identity('teacher')
  assert.equal((await rows('select id from public.students where id=$1', [otherStudent])).length, 0)
  assert.equal((await rows('select student_id from public.attendance_records where student_id=$1', [otherStudent])).length, 0)
  assert.equal((await rows('select * from public.subject_attendance')).length, 1)
  assert.deepEqual(await dailyEvidence(), legacy)
})

test('no-tap enrolled student can be Present, Late and Absent; unenrollment retains teacher history', async () => {
  const a = await schedule()
  await installEnrollment()
  await identity('teacher'); await confirm(a, 'Present')
  for (const status of ['Late', 'Absent']) {
    const old = (await rows('select * from public.subject_attendance'))[0]
    await confirm(a, status, student, old.confirmed_at)
  }
  await identity('admin'); await enroll(a, [])
  await identity('teacher')
  const history = await rows('select * from public.subject_attendance')
  assert.equal(history[0].attendance_status, 'Absent')
  await rejected(() => confirm(a, 'Present', student, history[0].confirmed_at), /not enrolled/)
  assert.deepEqual(await dailyEvidence(), legacy)
})

test('enrollment saves enforce admin, version, active students, active schedules and no direct writes', async () => {
  const a = await schedule()
  await installEnrollment()
  for (const role of ['teacher','student','otherTeacher']) {
    await identity(role)
    await rejected(() => enroll(a, []), /administrator/)
    await rejected(() => rows('update public.subject_enrollments set active=false'), /permission/)
  }
  await identity('admin')
  await rejected(() => rows('insert into public.subject_enrollments values($1,$2,true)', [a, otherStudent]), /permission/)
  await rejected(() => enroll(a, [999999]), /active students/)
  await rejected(() => enroll(a, [null]), /active students/)
  await rejected(() => enroll(a, null), /active students/)
  await enroll(a, [student, student])
  await rejected(() => enroll(a, []), /Enrollment changed/)
  assert.equal((await rows('select * from public.subject_enrollments where active')).length, 1)
  await db.exec('reset role')
  await rows("update public.students set status='archived' where id=$1", [otherStudent])
  await identity('admin'); await rejected(() => enroll(a, [otherStudent], 2), /active students/)
  await rows('select public.retire_subject_schedule($1)', [a])
  await rejected(() => enroll(a, [], 2), /active subject schedule/)
})

test('enrollment rollback disables writes without erasing data and reapply restores access', async () => {
  const a = await schedule()
  await installEnrollment()
  await identity('teacher'); await confirm(a)
  await db.exec('reset role')
  const history = await rows('select * from public.subject_attendance')
  const roster = await rows('select * from public.subject_enrollments')
  const sql = await readFile(new URL('../supabase/rollback_subject_enrollment.sql', import.meta.url), 'utf8')
  await db.exec(sql.replace(/^begin;$/m, '').replace(/^commit;$/m, ''))
  await identity('admin'); await rejected(() => enroll(a, []), /permission/)
  await identity('teacher'); await rejected(() => confirm(a), /permission/)
  await installEnrollment()
  assert.deepEqual(await rows('select * from public.subject_attendance'), history)
  assert.deepEqual(await rows('select * from public.subject_enrollments'), roster)
  await identity('teacher'); await confirm(a)
  assert.deepEqual(await dailyEvidence(), legacy)
})

test('assignment placement edits preserve explicit enrollment and inactive teachers lose enrolled access', async () => {
  const a = await schedule()
  await installAssignmentSync()
  await installEnrollment()
  await saveAssignment({section:'21003',campus:'MV Campus'})
  await identity('teacher')
  await confirm(a, 'Present')
  assert.equal((await rows('select id from public.students where id=$1', [student])).length, 1)
  await db.exec('reset role')
  await rows("update public.teachers set status='inactive' where id=$1", [teacher])
  await identity('teacher')
  assert.deepEqual(await rows('select * from public.subject_enrollments'), [])
  assert.equal((await rows('select id from public.students where id=$1', [student])).length, 0)
  await rejected(() => confirm(a))
})

test('enrollment table inherits installed email verification requirement', async () => {
  await schedule()
  await db.exec('reset role')
  await db.exec("create function public.has_verified_email_session() returns boolean language sql stable as $$ select coalesce(current_setting('test.email_verified',true),'false')='true' $$")
  await installEnrollment()
  await identity('teacher')
  assert.deepEqual(await rows('select * from public.subject_enrollments'), [])
  await rows("select set_config('test.email_verified','true',true)")
  assert.equal((await rows('select * from public.subject_enrollments')).length, 1)
})
